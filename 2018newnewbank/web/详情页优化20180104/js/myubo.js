...
function scollTab(imgUrl,oUl,pageNum,oBox,aBtn) {
    ...
    var oImg = oUl.getElementsByTagName('lm-img');
...
//  oUl.style.width = 245 * aLi.length + 'px';
...
//  }
    ...
    for (var i = 0; i < aBtn.length; i++) {
        ...
        aBtn[i].onclick = function () {
            ...
            var d_next2 = 0;
            var obj1 = aBtn[1];
            var d_prev = -getWidth(aLi[0]) * (aLi.length - 2 - showNum);
            var d_prev2 = -getWidth(aLi[0]) * (aLi.length -1- showNum);
            console.log(d_prev,d_prev2)
            var obj2 = aBtn[0];
